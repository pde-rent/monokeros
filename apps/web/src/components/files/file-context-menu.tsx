...
export function FileContextMenu({
  ...
  const isRootDir =
    entry?.type === "directory" && entry.path.split("/").filter(Boolean).length === 1;
  ...
  const items = useMemo<ContextMenuItem[]>(() => {
    const list: ContextMenuItem[] = [
      ...
      { id: "sep-2", label: "", separator: true },
      {
        ...
        icon: TrashIcon,
        danger: true,
        disabled: !entry || isProtected || isRootDir,
      },
    ];
  ...
  return (
    <ContextMenu position={position} items={items} onSelect={handleSelect} onClose={onClose} />
  );
}
