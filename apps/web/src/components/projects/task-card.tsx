...
export const TaskCard = React.memo(function TaskCard({
  ...
  return (
    <Card
      ...
      {...listeners}
      onClick={handleClick}
      className={`bg-elevated p-3 rounded-sm ${isDragging ? "rotate-2 shadow-xl opacity-90" : ""}`}
    >
      <div className="flex items-start gap-2">
        <span className="mt-1">
          <ColorDot color={PRIORITY_COLORS[task.priority]} />
        </span>
        <span className="text-sm font-medium text-fg leading-tight">{task.title}</span>
      </div>
      ...
      <div className="mt-2 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {team && <StatusBadge label={team.name.split(" ")[0]} color={team.color} />}
        </div>
