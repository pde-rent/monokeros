...
const filterMembers = createTextFilter<Member>("name", "title", "specialization");
...
export function AgentTable({ members, search }: Props) {
  const { sortKey, sortDir, sortProps } = useSort<SortKey>("name");
  ...
  const sorted = useMemo(() => {
    ...
    items.sort((a, b) => {
      switch (sortKey) {
        ...
        case "status":
          return (
            ((MEMBER_STATUS_ORDER[a.status] ?? 4) - (MEMBER_STATUS_ORDER[b.status] ?? 4)) * dir
          );
        case "team": {
  ...
  if (members.length === 0) {
    return <EmptyState>No agents to display</EmptyState>;
  }
      ...
      <table className="w-full">
        <thead className="sticky top-0 bg-surface-2">
          <tr className="border-b border-edge">
            ...
            <SortHeader label="Team" column="team" {...sortProps} />
            <SortHeader label="Specialization" column="specialization" {...sortProps} />
              ...
              >
                ...
                <td className="px-3 py-2">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs capitalize text-fg-2">{member.status}</span>
                  </div>
                </td>
                <td className="px-3 py-2">
                  {team && (
                ...
                <td className="px-3 py-2">
                  <span className="text-xs text-fg-2">{member.specialization}</span>
                </td>
                <td className="px-3 py-2">
                  {currentTask ? (
                    <span className="text-xs text-fg">{currentTask.title}</span>
                  ) : (
