...
sequenceDiagram
    ...
    LLM-->>OC: SSE chunks (delta.content, tool_calls)
    OC-->>API: DaemonEvents (status, content, tool_start, done)
    API-->>WS: stream-start
...
flowchart TD
    ...
    C -- Yes --> E[OpenClawService.streamMessage]
    E --> F[Service calls LLM via SSE]
    F --> G{Tool calls needed?}
    G -- Yes --> H[Execute tools up to 5 rounds]
    H --> F
    G -- No --> I[Parse SSE response]
    I --> J[Yield DaemonEvent objects]
...
The OpenClaw service yields `DaemonEvent` objects as it parses the SSE stream from the LLM:
...
| **Agent message** | Generated by the AI agent via the [OpenClaw service](../technical/daemon.md). Stored with `role: "assistant"`. |
...
The OpenClaw service maintains a bounded conversation history per agent (configured by `DAEMON_MAX_HISTORY`), keeping the system prompt plus the most recent messages to stay within LLM context limits.

## Related Documentation

- [WebSocket Protocol](../technical/websocket.md) -- Connection and event details
- [OpenClaw Service](../technical/daemon.md) -- How the agent runtime handles LLM calls
