/**
 * Deterministic pixel-art avatar generator. Zero dependencies.
 * Based on bit-face by Nemethe (https://github.com/Nemethe/bit-face) — MIT.
 */
import sprites from './sprites.json';

export type SkinTone =
  | 'light' | 'fair' | 'sand' | 'peach' | 'golden' | 'honey'
  | 'olive' | 'tan' | 'caramel' | 'brown' | 'umber' | 'espresso';
...
export const SKIN_TONES: Record<SkinTone, string> = {
  light: '#fde0c5', fair: '#f5ba81', sand: '#e8a96d', peach: '#dba070',
  golden: '#c8944e', honey: '#c6873a', olive: '#b07840', tan: '#a0653a',
  caramel: '#8d5534', brown: '#714228', umber: '#5a3420', espresso: '#3b1f0f',
};

// Skin anchor RGB dark→light for interpolation
const SA = [[59,31,15],[90,52,32],[113,66,40],[141,85,52],[160,101,58],[176,120,64],[198,135,58],[200,148,78],[219,160,112],[232,169,109],[245,186,129],[253,224,197]];

const HAIR = ['#2f2e2b','#23201b','#171410','#090806','#2C222B','#71635A','#A56B46','#B55239','#91553D','#533D32','#3B3024','#554838','#4E433F','#504444','#6A4E42','#D4A769','#C6A55A','#E8C97A','#B89B5E'];
const NEUTRALS = ['#ffffff','#e1e1e1','#1f1f1f','#000000'];
const COLORS = ['#6F1E51','#833471','#B53471','#ED4C67','#5758BB','#9980FA','#D980FA','#FDA7DF','#1B1464','#0652DD','#1289A7','#12CBC4','#006266','#009432','#A3CB38','#C4E538','#EA2027','#EE5A24','#F79F1F','#FFC312'];
const BGS: [string, string][] = [['#09d2ff','#ceeff7'],['#1b9e96','#cdf7f5'],['#b33c96','#e2b0d6'],['#ccd362','#e9ecb7'],['#b1ee6f','#d6f2b8']];

type Cat = 'skin' | 'hair' | 'shirt' | 'top';
const LAYERS: [string, Cat | null][] = [
  ['body/body','skin'],['body/shadow',null],['clothes/shirts','shirt'],
  ['clothes/tops','top'],['head/head','skin'],['head/eyes',null],
  ['head/mouth',null],['head/hair_female','hair'],['head/hair_male','hair'],
  ['head/beard','hair'],['head/addons',null],
];
const GENDER_LAYER: Record<string, 1|2> = { 'head/hair_male': 1, 'head/beard': 1, 'head/hair_female': 2 };
const OPTIONAL: Record<string, number> = { 'head/beard': 40, 'head/addons': 15 };

// PRNG
function djb2(s: string) { let h = 5381; for (let i = 0; i < s.length; i++) h = ((h << 5) + h + s.charCodeAt(i)) | 0; return h >>> 0; }
...
function resolve(path: string): string | Record<string, unknown> | null {
  ...
  for (const p of path.split('/')) { if (c && typeof c === 'object' && p in c) c = (c as any)[p]; else return null; }
  return c as any;
}

// Color math
const hex2rgb = (h: string) => { const n = parseInt(h.slice(1), 16); return [(n >> 16) & 0xff, (n >> 8) & 0xff, n & 0xff]; };
const rgb2hex = (c: number[]) => `#${c.map(v => cl(v).toString(16).padStart(2, '0')).join('')}`;
const lum = (h: string) => { const [r, g, b] = hex2rgb(h); return (0.299 * r + 0.587 * g + 0.114 * b) / 255; };
function shift(hex: string, amt: number) { return rgb2hex(hex2rgb(hex).map(c => c + Math.round((amt > 0 ? 255 - c : c) * Math.abs(amt)))); }
function rgbDist(a: string, b: string) { const [ar, ag, ab] = hex2rgb(a), [br, bg, bb] = hex2rgb(b); return Math.sqrt((ar - br) ** 2 + (ag - bg) ** 2 + (ab - bb) ** 2); }

function contrastPick(color: string, ref: string, palette: string[], r: () => number, min = 0.15): string {
...
function genSkin(r: () => number): string {
  ...
  const idx = t * (SA.length - 1), lo = Math.floor(idx), hi = Math.min(lo + 1, SA.length - 1), f = idx - lo;
...
export function generateAvatar(o: AvatarOptions): string {
  ...
  const skinHex = o.skinTone ? SKIN_TONES[o.skinTone] : genSkin(r);
  ...
  const filters = (Object.entries(colors) as [string, string][]).map(([c, hex]) =>
  ...
  for (const [path, cat] of LAYERS) {
    ...
    imgs.push(`<image href="data:image/png;base64,${data}" x="-1" y="0" width="40" height="40" image-rendering="pixelated"${f}/>`);
  }
