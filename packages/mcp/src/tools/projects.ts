...
import { enumValues, cleanUpdate, withResult } from "./utils";
...
export function registerProjectTools(server: McpServer, api: ApiClient) {
  server.tool(
    ...
    {
      status: z.enum(enumValues(TaskStatus)).optional().describe("Filter by project status"),
  ...
  server.tool(
    ...
    "Create a new project. Requires name and at least one project type.",
    {
      ...
      types: z.array(z.string().min(1)).describe("Project type tags"),
      phases: z
  ...
  server.tool(
    ...
    "Update project fields. All fields are optional.",
    {
      ...
      types: z.array(z.string().min(1)).optional().describe("New project types"),
      status: z.enum(enumValues(TaskStatus)).optional().describe("New status"),
    ...
    async ({ id, ...body }) => withResult(() => api.updateProject(id, cleanUpdate(body))),
  );
  ...
  server.tool(
    ...
    "Approve or reject an SDLC gate for a project phase.",
    {
      ...
      status: z.enum(enumValues(GateStatus)).describe("New gate status"),
